...
TRANSLATIONS = {
    "AR": {
        "title": "المنظومة الوطنية للرصد والإنذار المبكر",
    ...
    "EN": {
        "title": "National Early Warning & Command System",
        "login_tab": "Login",
        "signup_tab": "Sign Up",
        "user_label": "Username",
        "pass_label": "Password",
...
for key, val in {"authenticated": False, "user": None, "role": None,
                 "simulation_active": False, "sim_lat": 36.75, "sim_lon": 3.05,
                 "user_region": "ALL", "sat_data": []}.items():
                ...
                if user_in and pass_in:
                    ...
                    if success:
                        ...
                        if role == "Admin":
                            ...
                            st.session_state["user_region"] = "16_ALGIERS"
...
def get_fire_weather_index(lat: float, lon: float) -> dict:
    try:
        ...
        if fwi_score >= 70:
            danger_level, danger_ar, color = "extreme", "خطر شديد جداً", "#d32f2f"
        elif fwi_score >= 50:
            danger_level, danger_ar, color = "high", "خطر مرتفع", "#ff5722"
        elif fwi_score >= 30:
            danger_level, danger_ar, color = "moderate", "خطر متوسط", "#ff9800"
        elif fwi_score >= 15:
            danger_level, danger_ar, color = "low", "خطر منخفض", "#4caf50"
        else:
            danger_level, danger_ar, color = "minimal", "خطر ضئيل", "#8bc34a"

        return {
            "temperature": temp, "humidity": humidity, "wind_speed": wind_speed,
            "fwi_score": round(fwi_score, 1), "danger_level": danger_level,
            "danger_ar": danger_ar, "color": color
        }
    except Exception as e:
        logger.error(f"Open-Meteo API error: {e}")
        return {"temperature": "N/A", "humidity": "N/A", "wind_speed": "N/A",
                "fwi_score": 0, "danger_level": "unknown", "danger_ar": "غير محدد", "color": "#9e9e9e"}


def _fetch_single_region_prediction(lat: float, lon: float) -> dict:
    try:
        url = (
            f"https://api.open-meteo.com/v1/forecast?"
            f"latitude={lat}&longitude={lon}"
        ...
        if max_risk > 40:
            risk_level, risk_ar, color, emoji = "critical", "خطر حرج جداً متوقع", "#d32f2f", "🔴"
        elif max_risk > 25:
            risk_level, risk_ar, color, emoji = "high", "خطر مرتفع متوقع", "#ff5722", "🟠"
        elif max_risk > 15:
            risk_level, risk_ar, color, emoji = "moderate", "خطر متوسط متوقع", "#ff9800", "🟡"
        else:
            risk_level, risk_ar, color, emoji = "low", "حالة مستقرة", "#4caf50", "🟢"

        return {
            "risk_level": risk_level, "risk_ar": risk_ar, "risk_score": round(max_risk, 1),
            "color": color, "emoji": emoji, "high_risk_hours": len(risk_hours),
            "avg_temp": round(sum(hourly_temps) / max(len(hourly_temps), 1), 1),
            "avg_humidity": round(sum(hourly_hum) / max(len(hourly_hum), 1), 1),
            "avg_wind": round(sum(hourly_wind) / max(len(hourly_wind), 1), 1)
        }
    except Exception as e:
        logger.error(f"Predictive risk calculation failed: {e}")
        return {"risk_level": "unknown", "risk_ar": "غير محدد", "risk_score": 0,
                "color": "#9e9e9e", "emoji": "⚪", "high_risk_hours": 0,
                "avg_temp": 0, "avg_humidity": 0, "avg_wind": 0}


@st.cache_data(ttl=600)
def get_all_region_predictions() -> list:
    regions = [
    ...
    results = []
    for r in regions:
        pred = _fetch_single_region_prediction(r['lat'], r['lon'])
        results.append({
            "region": r['name'], "risk_level": pred['risk_ar'],
            "risk_score": pred['risk_score'], "emoji": pred['emoji'],
            "avg_temp": pred['avg_temp'], "avg_humidity": pred['avg_humidity']
        })
    return results
...
def get_all_alerts_for_analytics() -> pd.DataFrame:
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            if not c.fetchone():
                return pd.DataFrame()
            df = pd.read_sql_query("SELECT timestamp, region_id, hazard_type, confidence, source FROM alerts", conn)
            return df
    except Exception as e:
...
# Sidebar
# ==========================================
st.sidebar.markdown(f"<h2 style='text-align:center; color:#00f2fe;'>Command Center</h2>", unsafe_allow_html=True)
st.sidebar.markdown("---")
...
st.sidebar.markdown(f"<p style='color:#fff;'>**{t['username_label']}:** {current_user}</p>", unsafe_allow_html=True)
st.sidebar.markdown(f"<p style='color:#fff;'>**{t['role_label']}:** {current_role}</p>", unsafe_allow_html=True)
st.sidebar.markdown(f"<p style='color:#fff;'>**{t['region_label']}:** {current_region}</p>", unsafe_allow_html=True)
...
data_source = st.sidebar.radio(
    ...
    ["الكاميرات المباشرة (Live Cameras)", "الأقمار الصناعية (NASA FIRMS Satellite)"]
)
...
# Main Interface
# ==========================================
st.markdown("<h1 style='text-align:center;'>المركز الوطني للإنذار المبكر لحرائق الغابات</h1>", unsafe_allow_html=True)
...
with col1:
    if "الكاميرات" in data_source:
        ...
        st.info("يرجى تشغيل run_detector.py بشكل منفصل لتحليل البث الحي")
    else:
        st.subheader("الرصد الحراري عبر الأقمار الصناعية | NASA FIRMS")
        ...
        if st.button("تحديث وجلب النقاط الحرارية فوق الجزائر | Fetch Active Fires", use_container_width=True):
            with st.spinner("جاري سحب بيانات من الأقمار الصناعية..."):
                try:
                    ...
                    if api_key:
                        ...
                        if all_fires:
                            st.toast(f"تم جلب {len(all_fires)} نقطة حرارية!", icon="🛰️")
                except Exception as e:
...
with col2:
    ...
    alerts_data = get_alerts(st.session_state.get('user_region', 'ALL'))

    if alerts_data:
        df_display = pd.DataFrame(alerts_data, columns=["الوقت", "الولاية", "الخطر", "الدقة", "المصدر"])
...
# ==========================================
st.markdown("---")
st.subheader("خريطة الرصد الجغرافي الوطنية | Algerian Sovereign GIS")

m = folium.Map(location=[36.75, 3.05], zoom_start=6, tiles="OpenStreetMap", control_scale=True)

folium.Marker(
    location=[36.75, 3.05],
    popup="<b>المركز الوطني للقيادة والسيطرة</b>",
...
.leaflet-popup-tip { background: rgba(13, 17, 23, 0.9) !important; }
.leaflet-control-zoom a {
    background: rgba(13, 17, 23, 0.9) !important;
    color: #00f2fe !important;
    border-color: #30363d !important;
}
.leaflet-control-zoom a:hover { background: rgba(0, 242, 254, 0.2) !important; }
</style>
"""
...
if show_ndvi:
    try:
        folium.TileLayer(
            ...
            name="طبقة الصور الفضائية | Satellite Imagery",
            overlay=True,
            opacity=0.6
        ).add_to(m)
    except Exception:
...
if show_stations:
    ...
    if units:
        stations_group = folium.FeatureGroup(name="🚒 مراكز الحماية المدنية")
        for unit in units:
            try:
                lat, lon = float(unit[3]), float(unit[4])
                folium.Marker(
                    location=[lat, lon],
                    popup=f"<b>🚒 {unit[2]}</b><br>{unit[1]} - {unit[0]}<br>📞 {unit[5]}",
                    tooltip=unit[2],
...
if st.session_state.get('sat_data'):
    fires_group = folium.FeatureGroup(name="🔥 النقاط الحرارية")
    for fire in st.session_state['sat_data']:
        conf = str(fire.get('confidence', 'nominal')).lower()
        color = 'red' if conf == 'high' else ('orange' if conf == 'nominal' else 'yellow')
        popup_text = f"<b>🔥 نقطة حرارية</b><br>الموقع: {fire['lat']:.4f}°N, {fire['lon']:.4f}°E<br>الثقة: {fire.get('confidence', 'N/A')}"
        folium.Marker(
            [fire['lat'], fire['lon']],
...
if st.session_state.get("simulation_active"):
    ...
    sim_group = folium.FeatureGroup(name="🔥 محاكاة الانتشار")
    folium.Polygon(
        locations=[[sim_lat_val, sim_lon_val], [sim_lat_val + 0.1, sim_lon_val + 0.1], [sim_lat_val + 0.15, sim_lon_val]],
        color="red", fill=True, fill_color="red", fill_opacity=0.4,
        popup="منطقة الخطر العاجل (1 ساعة)"
    ).add_to(sim_group)
    folium.Polygon(
        locations=[[sim_lat_val, sim_lon_val], [sim_lat_val + 0.2, sim_lon_val + 0.2], [sim_lat_val + 0.25, sim_lon_val - 0.1]],
        color="orange", fill=True, fill_color="orange", fill_opacity=0.25,
...
legend_html = """
<div style="position:fixed; bottom:50px; left:50px; z-index:1000; background:rgba(13, 17, 23, 0.9); color:#00f2fe; padding:15px; border-radius:12px; border:1px solid #30363d; min-width:180px;">
    <b style="color:#fff; font-size:14px;">دليل الخريطة</b><hr style="border-color:#30363d; margin:8px 0;">
    <span style="color:#ff6b6b">🔥 خطر مرتفع</span><br>
    <span style="color:#ffaa00">🔥 خطر متوسط</span><br>
    <span style="color:#00f2fe">🚒 وحدة الحماية المدنية</span><hr style="border-color:#30363d; margin:8px 0;">
    <b style="color:#fff; font-size:12px;">محاكاة الانتشار:</b><br>
    <span style="color:#ff4444">■</span> منطقة الخطر (1 ساعة)<br>
    <span style="color:#ff9800">■</span> نطاق الإخلاء (3 ساعات)
</div>
"""
...
# ==========================================
st.markdown("---")
st.subheader("مؤشر مخاطر الحرائق الجوي | Fire Weather Index (FWI)")

col_fwi1, col_fwi2, col_fwi3, col_fwi4 = st.columns(4)
fwi_data = get_fire_weather_index(36.25, 3.05)

with col_fwi1:
    st.metric("الحرارة | Temperature", f"{fwi_data['temperature']}°C")
with col_fwi2:
...
with col_pred1:
    predictions = get_all_region_predictions()
...
with col_pred2:
    if predictions:
        ...
        <div style="padding:15px; border-radius:10px; background-color:#1a1a2e; color:white;">
            <h3 style="text-align:center; color:#ff5722">تنبيه التنبؤ المبكر</h3>
        ...
        if max_risk_pred['risk_score'] > 40:
            ...
            st.warning("يُنصح بمراقبة أطقم الإطفاء والاستعداد للتدخل السريع")
        else:
...
if not df_alerts.empty:
    ...
    with col_chart1:
        province_data = df_alerts.groupby('region_id').size().reset_index(name='count')
        fig_province = px.bar(province_data, x='region_id', y='count',
                              title='توزيع الإنذارات حسب الولايات',
                              color='count', color_continuous_scale='Reds')
        st.plotly_chart(fig_province, use_container_width=True)

    with col_chart2:
        if 'hazard_type' in df_alerts.columns:
            fig_hazard = px.pie(df_alerts.groupby('hazard_type').size().reset_index(name='count'),
                                names='hazard_type', values='count',
                                title='توزيع الإنذارات حسب النوع',
    ...
    if 'timestamp' in df_alerts.columns:
        ...
        daily = df_alerts.groupby('date').size().reset_index(name='count')
        fig_timeline = px.line(daily, x='date', y='count',
                               title='الإنذارات اليومية',
                               labels={'date': 'التاريخ', 'count': 'العدد'})
        st.plotly_chart(fig_timeline, use_container_width=True)
else:
    st.info("لا توجد بيانات كافية للتحليل | No alert data available")
        ...
        with st.spinner("جاري إنشاء التقرير..."):
            try:
                ...
                if alerts_for_report:
                    ...
                    for alert in alerts_for_report[:5]:
                        pdf.cell(0, 7, f"Time: {alert[0]} | Region: {alert[1]} | Type: {alert[2]} | Conf: {alert[3]}", ln=True)

                pdf_bytes = bytes(pdf.output())
                st.download_button(label="اضغط للتحميل | Click to Download", data=pdf_bytes,
...
<div style='text-align:center; padding:20px; background:linear-gradient(90deg, #1e1e1e, #2a2a2a); border-radius:10px; margin-top:20px;'>
    <p style='color:#00f2fe; font-weight:bold; font-size:14px;'>المركز الوطني للإنذار المبكر لحرائق الغابات</p>
